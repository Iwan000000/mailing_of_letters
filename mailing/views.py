...
class MailingCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        """
        Добавление названия рассылки (если пользователь не добавлял). Для создания рассылки мы используем formset.
        """
        self.object = form.save()
        self.object.user = self.request.user
        if self.object.name is None:
            self.object.name = f'Почтовое отправление №{self.object.pk}'
        self.object.save()
...
class MailingDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        """
        Дополнительная информация о рассылке.
        """
...
class MailingUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        """
        Для обновления рассылки мы используем formset.
        """
        self.object = form.save()
        self.object.user = self.request.user
        self.object.save()
        ...
        if mailing_formset.is_valid() and client_formset.is_valid():
            ...
            client_formset.instance = self.object
            client_formset.save()
            if self.object.status != 'Отменить':
                self.object.status = check_status_mailing(self.object)
        else:
            return self.form_invalid(form)
...
def mailing_go(request, pk):
    """
    Данная функция предназначена для запуска выполнения текущей рассылк.
    """
...
def mailing_finish(request, pk):
    """
    Эта функция предназначена для завершения текущей рассылки..
    """
    mailing = Mailing.objects.get(pk=pk)
    if mailing.user == request.user:
...
def mailing_change_status(request, pk):
    """
    Эта функция для изменения статуса рассылки (Отменено/Готово). Только для модераторов.
    """
    mailing = Mailing.objects.get(pk=pk)
    if mailing.status == 'Отменить':
        mailing.status = check_status_mailing(mailing)
    else:
        mailing.status = 'Отменить'
    mailing.save()
    return redirect(request.META.get('HTTP_REFERER'))
...
def mailing_again(request, pk):
    """
    Эта функция предназначена для перезапуска рассылки.
    """
    mailing = Mailing.objects.get(pk=pk)
    if mailing.user == request.user:
        mailing.status = 'Не готов'
        mailing.save()
        mailing.status = check_status_mailing(mailing)
